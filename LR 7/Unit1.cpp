	...
	if (FileOpenDialog1->Execute()) { //Проверка открытия файла
		...
		for (int i = 0, j = 0; j < (ts->Count)/4; i+=4, j++) {
			...
			if (ts->Strings[i + 3] == "Free") {
				...
				Form1->ComboBox2->Items->Add(ts->Strings[i + 2]);//Добавляем в ComboBox
			} else {
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Button6Click(TObject *Sender)//Обработчик событий Button6. В нем все так же, как и в Button1
{
